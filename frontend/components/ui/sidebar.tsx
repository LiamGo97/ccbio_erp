...
function SidebarProvider({
  ...
  // Initialize with defaultOpen to ensure server and client match on first render
  ...
  // 초기 렌더링 시 서버와 클라이언트를 일치시키기 위해 isHydrated가 false일 때는 defaultOpen 사용
  const effectiveOpen = isHydrated ? open : defaultOpen
...
function Sidebar({
    ...
    return (
      <Sheet open={openMobile} onOpenChange={setOpenMobile} {...props}>
        <SheetContent
          ...
          data-slot="sidebar"
          data-mobile="true"
          className="bg-background text-foreground w-(--sidebar-width) p-0 [&>button]:hidden [&_[data-sidebar=menu-button]]:text-base [&_[data-sidebar=menu-button]]:h-10 [&_[data-sidebar=menu-button]]:px-3"
          style={
            {
  ...
  // 서버와 클라이언트 초기 렌더링을 일치시키기 위해 항상 "expanded"로 시작
  const displayState = isMounted ? state : "expanded"
  const displayCollapsible = isMounted && state === "collapsed" ? collapsible : ""
