...
interface DrawerContentProps extends React.ComponentProps<typeof DrawerPrimitive.Content> {
  /** true면 반투명 배경(overlay) 미표시. 두 개 이상 drawer 동시 사용 시 상호작용 가능 */
  noOverlay?: boolean;
}
  ...
  return (
    <DrawerPortal data-slot="drawer-portal">
      ...
      <DrawerPrimitive.Content
        ...
        className={cn(
          "group/drawer-content bg-background fixed z-50 flex h-auto min-h-0 flex-col select-text",
...
function DrawerFooter({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn(
        "mt-auto flex flex-col gap-2 p-4 sm:flex-row sm:items-center sm:justify-end sm:gap-3",
        className
      )}
      {...props}
    />
