...
import { Eye, EyeOff, Filter, Download, Layers } from 'lucide-react';
...
import { InboundPendingDetailDrawer } from '@/components/inbound/inbound-pending-detail-drawer';
import { SalesInboundContainerGroupsSheet } from '@/components/inbound/sales-inbound-container-groups-sheet';
...
export function InboundPendingPageContent({ instanceId }: { instanceId: string }) {
  const isSalesInboundPending = instanceId === 'sales-inbound-pending';
  ...
  const [containerGroupsSheetOpen, setContainerGroupsSheetOpen] = React.useState(false);
  const [containerGroupsSheetOrder, setContainerGroupsSheetOrder] = React.useState<TradeOrder | null>(null);
  ...
  // eslint-disable-next-line react-hooks/exhaustive-deps -- 옵션 길이만으로 기본 전체 선택 동기화 (창고 필터와 동일)
  ...
  const sortedOrders = React.useMemo(() => {
    ...
    const isDate = ['etaDate', 'orderDate', 'createdAt', 'updatedAt'].includes(sortBy);
    const isNum = [
      'containerCount',
  ...
  const openContainerGroupsSheet = React.useCallback((order: TradeOrder) => {
    setContainerGroupsSheetOrder(order);
    setContainerGroupsSheetOpen(true);
  }, []);

  const invalidateInboundPendingLists = React.useCallback(async () => {
  ...
  const handleExcludeOrderFromList = React.useCallback(
    async (order: TradeOrder, exclude: boolean) => {
      ...
      try {
        ...
        await invalidateInboundPendingLists();
        void refetch();
      } catch (err: unknown) {
    ...
    [invalidateInboundPendingLists, refetch, toast],
  );
  ...
  const columns: ColumnDef<TradeOrder>[] = React.useMemo(() => [
    {
      ...
      cell: ({ row }) => {
        ...
        // 백엔드에서 변환된 이름이 있으면 사용, 없으면 프론트엔드에서 변환
    ...
    ...(isSalesInboundPending
      ? [
          {
            id: 'containerGroupsUi',
            header: () => <span className="whitespace-nowrap">컨 그룹</span>,
            enableSorting: false,
            cell: ({ row }) => {
              const order = row.original;
              const n = order.containers?.filter((c) => c?.id).length ?? 0;
              return (
                <div className="flex justify-center" onClick={(e) => e.stopPropagation()}>
                  <Button
                    ...
                    disabled={n === 0}
                    onClick={() => openContainerGroupsSheet(order)}
                  >
            ...
            size: 88,
            meta: { align: 'center' as const, headerLabel: '컨 그룹' },
          } satisfies ColumnDef<TradeOrder>,
        ]
      : []),
    {
  ...
  ], [getCodeName, salesGradeCodes, tradeStatusCodes, excludeActionOrderId, handleExcludeOrderFromList, isSalesInboundPending, openContainerGroupsSheet]);

  const handleScheduleInbound = (order: TradeOrder) => {
    setSelectedTradeOrderForInbound(order);
    setInboundEditDrawerOpen(true);
    // 상세정보 drawer는 유지 (닫지 않음)
  };
  ...
  const handleDownloadCsv = React.useCallback(() => {
    ...
    a.href = url;
    a.download = `${instanceId === 'sales-inbound-pending' ? '입고대기_신규' : '입고대기'}_${new Date().toISOString().slice(0, 10)}.csv`;
    a.click();
    URL.revokeObjectURL(url);
  }, [sortedOrders, tradeStatusCodes, getCodeName, salesGradeCodes, instanceId]);
      ...
      <div className="space-y-4 pb-4">
        <div className="flex flex-col gap-3">
          <div className="flex items-start justify-between gap-2">
            <div>
              <h1 className="text-2xl font-bold tracking-tight">입고 대기</h1>
              <p className="hidden text-muted-foreground md:block">
                {isSalesInboundPending
                  ? '입고 대기 BL을 조회합니다. 컨 그룹 구성은 화면에서만 설정하며 서버에 저장되지 않습니다.'
                  : '서류 처리 완료된 주문(BL) 정보를 조회합니다.'}
              </p>
            </div>
        ...
        <DataTable
          ...
          excludeRowDecorationColumnIds={
            isSalesInboundPending ? ['excludeAction', 'containerGroupsUi'] : ['excludeAction']
          }
          filterControls={
            <div className="flex flex-wrap items-center gap-3">
              <div className="flex items-center gap-2">
                <Label htmlFor={`${instanceId}-searchFilter`} className="whitespace-nowrap text-sm font-medium text-muted-foreground">검색</Label>
                <Input
                  id={`${instanceId}-searchFilter`}
                  value={searchFilter}
                    ...
                    <div className="space-y-3">
                      <div className="space-y-2">
                        <div className="flex items-center space-x-2 cursor-pointer hover:bg-muted/50 p-2 rounded">
                          <Checkbox
                            id={`${instanceId}-product-filter-all`}
                            checked={
                              availableProductCodes.length === 0 ||
                              selectedProducts.size === availableProductCodes.length
                            }
                          ...
                          />
                          <Label htmlFor={`${instanceId}-product-filter-all`} className="text-sm font-medium cursor-pointer flex-1">
                            전체
                          </Label>
                        </div>
                        {availableProductCodes.map((code) => (
                          <div key={code} className="flex items-center space-x-2 cursor-pointer hover:bg-muted/50 p-2 rounded">
                            <Checkbox
                              id={`${instanceId}-product-filter-${code}`}
                            ...
                            />
                            <Label htmlFor={`${instanceId}-product-filter-${code}`} className="text-sm font-medium cursor-pointer flex-1">
                ...
                <Popover>
                  ...
                  <PopoverContent className="w-56 p-3 max-h-[28rem] overflow-y-auto" align="start">
                    <div className="space-y-2">
                      <div className="flex items-center space-x-2 cursor-pointer hover:bg-muted/50 p-2 rounded">
                        <Checkbox
                          id={`${instanceId}-destination-filter-all`}
                          checked={
                            destinationFilterOptions.length === 0 ||
                            selectedDestinationValues.size === destinationFilterOptions.length
                          }
                        ...
                        />
                        <Label htmlFor={`${instanceId}-destination-filter-all`} className="text-sm font-medium cursor-pointer flex-1">
                          전체
                        </Label>
                      </div>
                      {destinationFilterOptions.map((opt) => (
                        ...
                        >
                          <Checkbox
                            id={`${instanceId}-destination-filter-${encodeURIComponent(opt.value)}`}
                          ...
                          />
                          <Label
                            htmlFor={`${instanceId}-destination-filter-${encodeURIComponent(opt.value)}`}
              ...
              <div className="ml-auto flex items-center gap-2">
                ...
                />
                <Label htmlFor={`${instanceId}-includeExcluded`} className="text-sm font-medium cursor-pointer whitespace-nowrap">제외된 BL 포함</Label>
              </div>
            </div>
          }
        />

        <InboundPendingDetailDrawer
          ...
          onScheduleInbound={handleScheduleInbound}
          ...
          tradeOrder={selectedTradeOrderForInbound}
            ...
            onSubmit={async (data) => {
              ...
              try {
                ...
                // 영업 상태 업데이트 (별도 API 호출)
                await api.put(`/trade/contracts/orders/${selectedTradeOrderForInbound.id}`, {
        ...
        <SalesInboundContainerGroupsSheet
          open={containerGroupsSheetOpen}
          onOpenChange={(open) => {
            setContainerGroupsSheetOpen(open);
            if (!open) {
              setContainerGroupsSheetOrder(null);
            }
          }}
          tradeOrder={containerGroupsSheetOrder}
        />
