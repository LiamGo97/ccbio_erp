...
import {
  useSalesInventoryConfirmedByBlPacking,
  type SalesInventoryConfirmedByBlPackingRow,
...
import { SalesInventoryConfirmedDetailDrawer } from '@/components/inventory/sales-inventory-confirmed-detail-drawer';
import { ReturnStatusCountBadges } from '@/components/inventory/return-status-badge';
...
export type SalesInventoryConfirmedPageContentProps = {
  instanceId: string;
...
export function SalesInventoryConfirmedPageContent({
  instanceId,
  embedded = false,
  title = '입고확정재고',
  description = 'BL·패킹 단위로 베일·중량을 합산합니다. 반납여부는 컨테이너마다 다를 수 있어 행을 클릭하면 컨별 목록에서 확인·수정할 수 있습니다.',
}: SalesInventoryConfirmedPageContentProps) {
  ...
  const [selectedRow, setSelectedRow] = useState<SalesInventoryConfirmedByBlPackingRow | null>(null);
  ...
  const returnStatusParam = useMemo(() => {
    if (selectedReturnStatuses.size === 0) return [];
    return Array.from(selectedReturnStatuses);
  }, [selectedReturnStatuses]);

  const { data: rowsRaw = [], isLoading: isRowsLoading } = useSalesInventoryConfirmedByBlPacking({
    search: search || undefined,
    productNames: productsParam,
    inventoryStatus: inventoryStatusParam.length > 0 ? inventoryStatusParam : undefined,
    returnStatus: returnStatusParam.length > 0 ? returnStatusParam : undefined,
  });
  ...
  const sortedRows = useMemo(() => {
    return [...rows].sort((a, b) => {
      let aValue: string | number = a[sortBy as keyof SalesInventoryConfirmedByBlPackingRow] as string | number;
      let bValue: string | number = b[sortBy as keyof SalesInventoryConfirmedByBlPackingRow] as string | number;
  ...
  const formatQuantity = useCallback(
    (available: number | null, total: number | null, sold: number | null, maxDecimals: number) => {
      if (available == null || total == null) return { text: '-', hasSales: false as const };
      ...
      return { text: fmt(total), hasSales: false as const, total: fmt(total) };
    },
    [],
  );
  ...
  const columns: ColumnDef<SalesInventoryConfirmedByBlPackingRow>[] = useMemo(
    () => [
      {
        accessorKey: 'bl',
        header: 'BL',
      ...
      },
      {
        ...
        cell: ({ row }) => {
          const r = row.original;
          if (r.customsDateMixed) return <span className="text-muted-foreground">혼합</span>;
          const value = r.customsDate;
      ...
      },
      {
        ...
        cell: ({ row }) => {
          const r = row.original;
          if (r.inboundDtDateMixed) return <span className="text-muted-foreground">혼합</span>;
          const value = r.inboundDtDate;
          return <div className="truncate" title={value || undefined}>{value || '-'}</div>;
        },
        size: 95,
      },
      ...
      },
      {
        ...
        header: '반납여부',
        cell: ({ row }) => (
          <ReturnStatusCountBadges counts={row.original.returnStatusCounts} />
        ),
      },
      {
  ...
  const pageBody = (
    <div className="space-y-4 pb-4">
          ...
          filterControls={
            <div className="flex flex-wrap items-center gap-3">
              <div className="flex items-center gap-2">
                <Label htmlFor="salesInvSearch" className="whitespace-nowrap text-sm font-medium text-muted-foreground">
                  검색
                </Label>
                <Input
                  id="salesInvSearch"
                    ...
                    <div className="space-y-2">
                      {returnStatusOptions.map((opt) => (
                        <div key={opt.value} className="flex items-center space-x-2 p-2 rounded hover:bg-muted/50">
                          <Checkbox
                            checked={selectedReturnStatuses.has(opt.value)}
                            onCheckedChange={(checked) => {
                              const next = new Set(selectedReturnStatuses);
                              if (checked === true) next.add(opt.value);
                              else next.delete(opt.value);
                              setSelectedReturnStatuses(next);
                              setPage(1);
                            }}
                          />
      ...
      <SalesInventoryConfirmedDetailDrawer
