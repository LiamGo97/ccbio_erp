...
import { ReturnStatusBadge, ReturnStatusCountBadges } from '@/components/inventory/return-status-badge';
...
interface SalesInventoryConfirmedDetailDrawerProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  row?: SalesInventoryConfirmedByBlPackingRow | null;
}
...
export function SalesInventoryConfirmedDetailDrawer({
  open,
  onOpenChange,
  row,
}: SalesInventoryConfirmedDetailDrawerProps) {
  ...
  const invalidateRelatedQueries = React.useCallback(() => {
    void queryClient.invalidateQueries({ queryKey: ['trade-contracts', 'sales', 'inventory-confirmed'] });
    void queryClient.invalidateQueries({
      queryKey: ['trade-contracts', 'sales', 'inventory-confirmed', 'sales-linked'],
    });
    void queryClient.invalidateQueries({ queryKey: ['trade-contracts', 'containers', 'by-bl-packing'] });
    void queryClient.invalidateQueries({ queryKey: ['trade-contracts', 'containers', 'confirmed'] });
  ...
  return (
    <>
            ...
            <div className="flex items-center justify-between">
              <div className="flex-1 min-w-0">
                <div className="flex flex-wrap items-center gap-2">
                  <DrawerTitle className="truncate">입고 확정 재고 상세</DrawerTitle>
                  {row?.bl ? (
                ...
                <DrawerDescription className="mt-1">
                  BL·패킹 재고, 연결 판매·예약, 컨테이너 물리 정보를 확인합니다.
                </DrawerDescription>
              </div>
              <DrawerClose asChild>
            ...
            >
                    ...
                    <div className="mb-4 rounded-lg border bg-muted/30 px-4 py-3 text-sm">
                      ...
                      <p className="mt-2 flex flex-wrap items-center gap-x-4 gap-y-1 text-muted-foreground">
                        ...
                        <span className="inline-flex items-center gap-1">
                          반납:
                          <ReturnStatusCountBadges counts={row.returnStatusCounts} />
                        </span>
                      </p>
              ...
              >
                <p className="text-xs text-muted-foreground mb-3">
                  컨테이너별 STO·작업비·비고 등을 확인합니다. 반납여부 변경은 행을 클릭해 상세에서
                  처리합니다.
                </p>
                {isContainersLoading ? (
                  <div className="flex items-center justify-center py-12">
                    ...
                    <Table>
                      <TableHeader>
                        <TableRow className="bg-muted/50">
                          ...
                          <TableHead className="text-right">작업비(합)</TableHead>
                            ...
                            >
                              ...
                              <TableCell>
                                <ReturnStatusBadge
                                  ...
                                  name={c.returnStatusName}
                                  className="truncate max-w-[7rem]"
                                />
                              </TableCell>
                              <TableCell className="text-sm truncate max-w-[100px]" title={c.inboundWarehouseName ?? undefined}>
                                {c.inboundWarehouseName || '-'}
                              </TableCell>
                              <TableCell className="text-sm text-right tabular-nums">
                                {formatContainerMoney(c.stoCost)}
                              </TableCell>
                              <TableCell className="text-sm text-right tabular-nums">
                                {formatContainerMoney(c.dtCost)}
                              </TableCell>
                              <TableCell className="text-sm text-right tabular-nums">
                                {formatWorkFeeSum(c.workFee, c.onsiteWorkFee)}
                              </TableCell>
                              <TableCell
      ...
      <InventoryConfirmedDetailDrawer
        ...
        onInventoryAdjustmentSuccess={invalidateRelatedQueries}
      />
