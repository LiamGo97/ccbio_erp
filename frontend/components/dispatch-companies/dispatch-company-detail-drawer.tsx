...
import { useDispatchCompany, DispatchCompany } from '@/lib/hooks/use-dispatch-companies';
import { Badge } from '@/components/ui/badge';
import { Label } from '@/components/ui/label';
import { Loader2, Edit, X } from 'lucide-react';

interface DispatchCompanyDetailDrawerProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  dispatchCompanyId?: number | null;
  onEdit?: (dispatchCompany: DispatchCompany) => void;
}

export function DispatchCompanyDetailDrawer({
  open,
  onOpenChange,
  dispatchCompanyId,
  onEdit,
}: DispatchCompanyDetailDrawerProps) {
  const { data, isLoading, refetch } = useDispatchCompany(dispatchCompanyId ?? undefined);

  // drawer가 열릴 때마다 데이터 갱신
  React.useEffect(() => {
    if (open && dispatchCompanyId) {
      refetch();
    }
  }, [open, dispatchCompanyId, refetch]);
        ...
        <DrawerHeader className="border-b border-border">
          <div className="flex items-start justify-between gap-4">
            <div className="space-y-1">
              <DrawerTitle>배차 업체 상세정보</DrawerTitle>
              <DrawerDescription>
                배차 업체의 기본 정보를 확인할 수 있습니다.
              </DrawerDescription>
            </div>
            <DrawerClose asChild>
