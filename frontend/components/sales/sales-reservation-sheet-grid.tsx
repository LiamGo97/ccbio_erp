...
import {
  ...
  VEHICLE_REQUESTED_CONTAINER_QTY_HINT_ROWS,
} from '@/lib/sales/vehicle-requested-container-qty';
import { computeBlOptionsDrivingSignature } from '@/lib/sales/sales-reservation-sheet-bl-driving';
import {
  useSheetBlOptionsForProduct,
  fetchSheetBlOptions,
  sheetBlOptionsQueryKey,
  SHEET_BL_OPTIONS_STALE_MS,
  SHEET_BL_OPTIONS_QUERY_ROOT,
} from '@/lib/hooks/use-sheet-bl-options-for-product';
...
const COL_BL = 4;
/** G열 요청수량 */
const COL_REQUESTED_QTY = 6;
/** H열 차량분류 */
const COL_VEHICLE = 7;
/** L열 단가 */
const COL_UNIT_PRICE = 11;
/** F열 담당연락처 — 화면에만 숨김(저장·API values 인덱스는 그대로) */
const COL_CONTACT = 5;
const SHEET_UI_HIDDEN_COLS: ReadonlySet<number> = new Set([COL_CONTACT]);
...
function isSheetNumericColumn(col: number): boolean {
  return col === COL_REQUESTED_QTY || col === COL_UNIT_PRICE;
}
...
/** `/consultations/search/company` 응답 — 예약 드로어와 동일 */
type SheetCompanySearchResult = {
  id: string;
  phone: string | null;
...
const DEFAULT_SHEET_COL_WIDTHS: readonly number[] = [
  104, 88, 120, 112, 168, 120, 120, 120, 120, 120, 120, 120, 120,
];
      ...
      {isSheetSelectColumn ? (
        <div
          className={cn(
            ...
            inRangeForBg ? 'bg-[#e8f0fe]' : rowStatusBgClass ?? 'bg-white',
...
type MemoSheetRowHeaderProps = {
  row: number;
  /** 왼쪽에 그리는 번호(필터 시 1부터 표시 순번, 없을 때는 실제 행과 동일) */
  headerDisplayNumber: number;
  /** 행 필터 적용 중이면 툴팁에 실제 행 번호를 덧붙임 */
  hasVisibleRowFilter: boolean;
  /** 저장 연동 시 줄번호 대신 표시(변경됨·성공·실패) */
  persistSaveOutcome?: 'dirty' | 'success' | 'error' | null;
  className: string;
  stablePointerDown: React.PointerEventHandler<HTMLTableCellElement>;
  stablePointerEnter: React.PointerEventHandler<HTMLTableCellElement>;
  stableContextMenu: React.MouseEventHandler<HTMLTableCellElement>;
...
const MemoSheetRowHeader = React.memo(function MemoSheetRowHeader({
  ...
  headerDisplayNumber,
  hasVisibleRowFilter,
  persistSaveOutcome = null,
  className,
  stablePointerDown,
  stablePointerEnter,
  stableContextMenu,
}: MemoSheetRowHeaderProps) {
  const physicalRowNo = row + 1;
  const rowNoLabel = hasVisibleRowFilter
    ? `표시 ${headerDisplayNumber}번 (${physicalRowNo}번 행)`
    : `행 ${physicalRowNo}번`;
  const persistTitle =
    persistSaveOutcome === 'dirty'
      ? '변경됨 — 저장 전 또는 저장 중'
      : persistSaveOutcome === 'success'
        ? '서버에 저장됨'
        : persistSaveOutcome === 'error'
          ? '저장 실패 — 토스트 확인'
          : null;
  const thTitle =
    persistTitle != null
      ? `${rowNoLabel}: ${persistTitle} · 행 전체 선택·드래그·우클릭`
      : `${rowNoLabel} · 행 전체 선택 · 드래그로 여러 행 · 우클릭 메뉴`;
  ...
  return (
    <th
      ...
      aria-label={
        persistSaveOutcome != null
          ? `${rowNoLabel}, ${persistTitle ?? '저장 상태'}`
          : rowNoLabel
      }
    >
      {persistSaveOutcome === 'dirty' ? (
        <span
          className="inline-flex size-full min-h-[28px] items-center justify-center text-amber-600 dark:text-amber-500"
          aria-hidden
        >
        ...
        <span
          className="inline-flex size-full min-h-[28px] items-center justify-center text-emerald-600 dark:text-emerald-500"
          aria-hidden
        >
          <Check
        ...
        <span
          className="inline-flex size-full min-h-[28px] items-center justify-center text-destructive"
          aria-hidden
        >
...
function columnHeaderLabel(index: number): string {
  if (index === 0) return '상품';
  if (index === 1) return '등급';
  if (index === 2) return '업체명';
  if (index === 3) return '상태';
  if (index === 4) return 'BL';
  if (index === 5) return '담당연락처';
  if (index === 6) return '요청수량(컨)';
  if (index === 7) return '차량분류';
  if (index === 8) return '상차일정';
  if (index === 9) return '도착일정';
  if (index === 10) return '비고';
  if (index === 11) return '단가';
  if (index === 12) return '참고';
 ...
 *   (삽입 직후 빈 줄이 곧바로 숨겨지지 않게). 값이 채워지면 그 열부터는 일반 필터와 동일.
 */
...
/**
 * 행 헤더 드래그로 잡힌 물리 범위 [r0,r1] 안에서, 필터로 실제 그려진 행만 골라낸다.
 * (숨겨진 행은 범위에 포함되어도 사용자가 헤더로 선택하지 않은 것과 동일하게 취급)
 */
function visibleRowIndicesInPhysicalRange(
  ...
  return visibleRows.filter((r) => r >= lo && r <= hi);
}
...
function remapProductFilterBypassAfterDeletingIndices(
  prev: Set<number>,
  deleted: Set<number>,
): Set<number> {
  const next = new Set<number>();
...
/** 선택 행 인덱스(비연속 가능)를 제거하고 아래 행 인덱스를 위로 당김 */
function remapCellsAfterDeletingRowIndices(
...
function remapCellsAfterDeletingRowRange(
  ...
  const list: number[] = [];
  for (let r = r0; r <= r1; r++) list.push(r);
  return remapCellsAfterDeletingRowIndices(prev, list);
}
...
function extractRowStrings(
  ...
  return Array.from({ length: COL_COUNT }, (_, c) => cells[cellKey(row, c)] ?? '');
}
...
function buildCopyTsv(
  ...
  for (let r = r0; r <= r1; r++) {
    const parts: string[] = [];
    for (let c = c0; c <= c1; c++) {
      parts.push(escapeCellForTsv(cellMap[cellKey(r, c)] ?? ''));
    }
...
/**
 * 같은 행 PUT 디바운스(ms). 너무 짧으면 셀 이동·짧은 멈춤마다 PUT이 나가
 * “셀마다 저장”처럼 느껴짐. 탭 숨김·언마운트 시에는 즉시 flush 됨.
 */
const PERSIST_ROW_DEBOUNCE_MS = 1200;
/** A열 저장 성공 체크 아이콘 자동 제거(ms) — 실패 표시는 다음 저장 시도 전까지 유지 */
const PERSIST_ROW_SAVE_UI_CLEAR_MS = 8000;
...
export interface SalesReservationSheetGridProps {
...
export const SalesReservationSheetGrid = React.forwardRef<
  SalesReservationSheetGridHandle,
  SalesReservationSheetGridProps
>(function SalesReservationSheetGrid(
  {
  ...
  const sheetBlLazyProduct =
    editing?.col === COL_BL
      ? (cells[cellKey(editing.row, 0)] ?? '').trim()
      : '';
  const sheetBlLazySalesGrade =
    editing?.col === COL_BL
      ? (cells[cellKey(editing.row, COL_SALES_GRADE)] ?? '').trim()
      : '';

  /** BL 편집 진입 + 상품만 있으면 목록 요청(등급은 있으면 쿼리에만 포함) */
  const sheetBlLazyEnabled =
    editing != null &&
    editing.col === COL_BL &&
    sheetBlLazyProduct !== '';

  const {
    data: sheetBlLazyData,
    isFetching: sheetBlLazyFetching,
    isSuccess: sheetBlLazySuccess,
    dataUpdatedAt: sheetBlLazyUpdatedAt,
  } = useSheetBlOptionsForProduct(
    sheetBlLazyProduct,
    sheetBlLazySalesGrade,
    sheetBlLazyEnabled,
  );

  React.useEffect(() => {
    if (!sheetBlLazyEnabled || !sheetBlLazySuccess || sheetBlLazyData === undefined) {
      return;
    }
    const p = sheetBlLazyProduct.trim();
    if (!p) return;
    const g = sheetBlLazySalesGrade.trim();
    const k = sheetBlCompositeKey(p, g);
    setSheetBlOptionsCache((prev) => ({ ...prev, [k]: sheetBlLazyData }));
  }, [
    sheetBlLazyEnabled,
    sheetBlLazySuccess,
    sheetBlLazyData,
    sheetBlLazyProduct,
    sheetBlLazySalesGrade,
    sheetBlLazyUpdatedAt,
  ]);

  const mergedBlOptionsByProductCode = React.useMemo(() => {
    if (Object.keys(sheetBlOptionsCache).length === 0) {
      return blOptionsByProductCode;
    }
    return { ...blOptionsByProductCode, ...sheetBlOptionsCache };
  }, [blOptionsByProductCode, sheetBlOptionsCache]);

  const sheetBlDropdownLoading =
    sheetBlLazyEnabled &&
    (sheetBlLazyFetching || !sheetBlLazySuccess);
  ...
  const getComboScrollViewportElement = React.useCallback((): HTMLElement | null => {
    ...
    const root =
      col === 0
        ? firstColScrollAreaRootRef.current
        : col === COL_COMPANY
  ...
  /** `visibleRows`는 아래에서 정의 — 행 삭제 등 조기 콜백에서 최신 값만 ref로 참조 */
  const visibleRowsRef = React.useRef<number[]>([]);
  ...
  const [rowPersistSaveUi, setRowPersistSaveUi] = React.useState<
    Record<number, 'dirty' | 'success' | 'error'>
  >({});
  const persistSuccessClearTimersRef = React.useRef<Map<number, number>>(new Map());

  const clearPersistSuccessTimerForRow = React.useCallback((row: number) => {
    const t = persistSuccessClearTimersRef.current.get(row);
    if (t != null) {
      clearTimeout(t);
      persistSuccessClearTimersRef.current.delete(row);
    }
  }, []);

  const clearRowPersistBadge = React.useCallback(
    (row: number) => {
      clearPersistSuccessTimerForRow(row);
      setRowPersistSaveUi((prev) => {
        if (prev[row] == null) return prev;
        const next = { ...prev };
        delete next[row];
        return next;
      });
    },
    [clearPersistSuccessTimerForRow],
  );

  const markRowPersistDirty = React.useCallback(
    (row: number) => {
      clearPersistSuccessTimerForRow(row);
      setRowPersistSaveUi((prev) => ({ ...prev, [row]: 'dirty' }));
    },
    [clearPersistSuccessTimerForRow],
  );

  const setRowPersistSaveOutcome = React.useCallback(
    (row: number, outcome: 'success' | 'error') => {
      clearPersistSuccessTimerForRow(row);
      setRowPersistSaveUi((prev) => ({ ...prev, [row]: outcome }));
      if (outcome === 'success') {
        const id = window.setTimeout(() => {
          persistSuccessClearTimersRef.current.delete(row);
          setRowPersistSaveUi((prev) => {
            if (prev[row] !== 'success') return prev;
            const next = { ...prev };
            delete next[row];
            return next;
          });
        }, PERSIST_ROW_SAVE_UI_CLEAR_MS);
        persistSuccessClearTimersRef.current.set(row, id);
      }
    },
    [clearPersistSuccessTimerForRow],
  );
  ...
  React.useEffect(() => {
    return () => {
      for (const tid of persistSuccessClearTimersRef.current.values()) {
        clearTimeout(tid);
      }
      persistSuccessClearTimersRef.current.clear();
    };
  }, []);
      ...
      void (async () => {
        try {
          if (job.gen === persistGenRef.current) {
            ...
            queryClient.invalidateQueries({ queryKey: [...SHEET_BL_OPTIONS_QUERY_ROOT] });
            setBlOptionsRefreshEpoch((e) => e + 1);
            setRowPersistSaveOutcome(job.row, 'success');
          }
        } catch {
          if (job.gen === persistGenRef.current) {
            setRowPersistSaveOutcome(job.row, 'error');
          }
        } finally {
          persistInFlightCountRef.current -= 1;
  ...
  }, [releasePersistPendingIfIdle, queryClient, setRowPersistSaveOutcome]);
  ...
  React.useEffect(() => {
    ...
    const t = window.setTimeout(() => {
      void (async () => {
        try {
          const { data } = await api.get<SheetCompanySearchResult[]>(
            '/consultations/search/company',
            { params: { keyword: q } },
          );
  ...
  React.useEffect(() => {
    if (!editing || editing.col !== 0) return;
    const t = window.setTimeout(() => {
  ...
  const schedulePersistRow = React.useCallback(
    ...
    [enqueuePersistJob, markRowPersistDirty, releasePersistPendingIfIdle],
  );
  ...
  const handleExcelExport = React.useCallback(() => {
    const aoa = srsBuildExportAoA({
      rowCount: rowCountRef.current,
      getCell: (r, c) => cellsRef.current[cellKey(r, c)] ?? '',
    });
    const stamp = format(new Date(), 'yyyyMMdd-HHmmss');
    srsDownloadXlsx(aoa, `판매예약-${stamp}.xlsx`);
    toast({
      title: '엑셀 다운로드',
  ...
  const deleteSelectedRowsFromContextMenu = React.useCallback(() => {
    ...
    const r0 = Math.min(a.row, f.row);
    const r1 = Math.max(a.row, f.row);
    const targets = visibleRowIndicesInPhysicalRange(
    ...
    if (targets.length === 0) {
      toast({
        title: '행을 삭제할 수 없습니다',
        description:
          '선택 범위에 표시 중인 행이 없습니다. 필터를 조정하거나 선택을 다시 잡아 주세요.',
        variant: 'destructive',
      });
      return;
    }
    ...
    productFilterBypassRowsRef.current = remapProductFilterBypassAfterDeletingIndices(
    ...
    const topDeleted = targets[0]!;
    const nextFocusRow = Math.max(0, Math.min(topDeleted, nextRowCount - 1));
    ...
    setCellsWithHistory((prev) => {
      ...
      const next = remapCellsAfterDeletingRowIndices(prev, targets);
      ...
      queueMicrotask(() => {
        ...
        const persistFrom =
          targets.length > 0 ? Math.min(...targets) : 0;
        /** 삭제 인덱스 미만은 행 번호·내용 그대로 — 재저장 생략 */
        for (let r = persistFrom; r <= newMax; r++) {
          schedulePersistRow(r, extractRowStrings(next, r));
        }
  ...
  const insertRowsAtFromContextMenu = React.useCallback(
    (insertAt: number, count: number, focusRow: number) => {
      ...
      setCellsWithHistory((prev) => {
        ...
        queueMicrotask(() => {
          if (!onPersistRow) return;
          /** 위쪽 물리 행(0..insertAt-1)은 인덱스·내용 동일 — 서버 재저장·토스트 불필요 */
  ...
  const clearSelectedRowsDataFromContextMenu = React.useCallback(() => {
    ...
    toast({
      title: '행 데이터 삭제됨',
      description: `${targets.length}개 행의 내용을 비웠습니다.`,
    });
  }, [
    commitEdit,
    onPersistRow,
      ...
      if (
        ...
        if (col === 0) {
          if (initialChar !== undefined && isAsciiPrintableKey(initialChar)) {
            setFirstColSearchTerm(initialChar);
  ...
  /** 행 헤더 우클릭 메뉴 라벨·삽입 위치 — 물리 범위가 아닌 표시 중인 행만 반영 */
  const rowHeaderContextBulkTargets = React.useMemo(() => {
    if (sheetContextMenu?.variant !== 'row-header') return null;
    const r0 = Math.min(anchor.row, focus.row);
    const r1 = Math.max(anchor.row, focus.row);
    const targets = visibleRowIndicesInPhysicalRange(visibleRows, r0, r1);
    const n = targets.length;
    return {
      targets,
      n,
      rTop: n ? Math.min(...targets) : r0,
      rBot: n ? Math.max(...targets) : r1,
    };
  }, [sheetContextMenu, anchor.row, focus.row, visibleRows]);

  /** 데이터 셀 우클릭「선택한 행 삭제」라벨용 */
  const cellContextVisibleRowCount = React.useMemo(() => {
    if (sheetContextMenu == null || sheetContextMenu.variant === 'row-header') {
      return null;
    }
    const r0 = Math.min(anchor.row, focus.row);
    const r1 = Math.max(anchor.row, focus.row);
    return visibleRowIndicesInPhysicalRange(visibleRows, r0, r1).length;
  }, [sheetContextMenu, anchor.row, focus.row, visibleRows]);

  /** 새로고침 직후 읽기 전용 BL(입고·ETA) — 필터로 보이는 행만, 상품+BL 있는 (상품,등급) 쌍 프리패치 */
  const sheetBlPrefetchPairs = React.useMemo(() => {
    const seen = new Set<string>();
    const pairs: { p: string; g: string }[] = [];
    for (const r of visibleRows) {
      const p = (cells[cellKey(r, 0)] ?? '').trim();
      const g = (cells[cellKey(r, COL_SALES_GRADE)] ?? '').trim();
      const bl = (cells[cellKey(r, COL_BL)] ?? '').trim();
      if (!p || !bl) continue;
      const ck = sheetBlCompositeKey(p, g);
  ...
  React.useEffect(() => {
    if (sheetBlPrefetchPairs.length === 0) return;
    let cancelled = false;
    const pairs = sheetBlPrefetchPairs;
    void (async () => {
      await Promise.all(
        pairs.map(({ p, g }) =>
          queryClient.ensureQueryData({
            queryKey: sheetBlOptionsQueryKey(p, g),
            queryFn: () => fetchSheetBlOptions(p, g),
            staleTime: SHEET_BL_OPTIONS_STALE_MS,
          }),
        ),
      );
      if (cancelled) return;
      setSheetBlOptionsCache((prev) => {
        const next = { ...prev };
        for (const { p, g } of pairs) {
          const data = queryClient.getQueryData<BuiltSheetBlOption[]>(
            sheetBlOptionsQueryKey(p, g),
          );
          if (data !== undefined) {
            next[sheetBlCompositeKey(p, g)] = data;
          }
        }
        return next;
      });
  ...
  }, [sheetBlPrefetchPairs, queryClient, blOptionsRefreshEpoch]);
  ...
  React.useEffect(() => {
    const onKeyDown = (e: KeyboardEvent) => {
      ...
      if (isSheetRangeClearKey(e)) {
        ...
        const c0 = Math.min(a.col, f.col);
        const c1 = Math.max(a.col, f.col);
        const rowTargets = visibleRowIndicesInPhysicalRange(visibleRows, r0, r1);
  ...
  const onCompanySearchKeyDown = (
    ...
    if (e.key === 'Enter') {
      ...
      if (companyActiveIndex > 0) {
        ...
        const label = (item?.companyName ?? '').trim();
        ...
        const label = (companySearchResults[0]?.companyName ?? '').trim();
        applyCompanyLabelAndCommit(label || draftRef.current.trim());
      } else {
        finishCommitAsTyped();
      }
  ...
  /** 상품·등급이 바뀐 경우에만 무거운 BL 필터/배열을 다시 만듦(`cells` 다른 열만 바뀌면 스킵) */
  const blOptionsDrivingSignature = React.useMemo(
    () =>
      computeBlOptionsDrivingSignature(
        rowCount,
        cells,
        cellKey,
        0,
  ...
  const blOptionsPerRowIndexed = React.useMemo(() => {
    ...
    const out: SheetBlOption[][] = new Array(rowCount);
    for (let r = 0; r < rowCount; r++) {
      const p = (c[cellKey(r, 0)] ?? '').trim();
      const grade = (c[cellKey(r, COL_SALES_GRADE)] ?? '').trim();
      const k = sheetBlCompositeKey(p, grade);
      out[r] = p ? (mergedBlOptionsByProductCode[k] ?? []) : [];
    }
    return out;
  }, [blOptionsDrivingSignature, rowCount, mergedBlOptionsByProductCode]);

  const sheetBlStoredByRow = React.useMemo(() => {
    const out: string[] = new Array(rowCount);
    for (let r = 0; r < rowCount; r++) {
      out[r] = cells[cellKey(r, COL_BL)] ?? '';
    }
    return out;
  }, [rowCount, cells]);
  ...
  const secondColSelectItems = React.useMemo(() => {
    ...
    if (editing?.col !== COL_BL) {
      return [empty];
    }
    const product = (cells[cellKey(editing.row, 0)] ?? '').trim();
    const grade = (cells[cellKey(editing.row, COL_SALES_GRADE)] ?? '').trim();
    const blList = product
      ? (mergedBlOptionsByProductCode[sheetBlCompositeKey(product, grade)] ?? [])
      : [];
    let list: (SheetProductOption | SheetBlOption)[] = [empty, ...blList];
    if (
      draft &&
      draft !== FIRST_COL_EMPTY &&
      !list.some((o) => o.value === draft)
    ...
    return list;
  }, [editing, draft, cells, mergedBlOptionsByProductCode]);
  ...
  const selectionStatusBar = React.useMemo(() => {
    ...
    const displayRowNum = (r: number) => {
      if (!hasVisibleRowFilter) return r + 1;
    ...
    return {
      ...
      hint: hasVisibleRowFilter
        ? '범위 지우기·행 메뉴 등은 표시 중인 행에만 적용됩니다. 왼쪽 번호는 필터 시 1부터이며, 저장·토스트는 실제 행 번호입니다.'
        : null,
    };
  }, [
    rangeBounds,
  ...
  React.useEffect(() => {
    ...
    const t = window.setTimeout(() => {
      if (col === 0) firstColSearchInputRef.current?.focus();
      else if (col === COL_COMPANY) companySearchInputRef.current?.focus();
      else if (col === COL_SALES_GRADE)
  ...
  return (
    ...
    >
      ...
      <div className="shrink-0 border-b border-border bg-muted/25 px-3 py-2">
        <div className="flex flex-wrap items-center gap-1 text-[11px] leading-snug text-muted-foreground">
          <span>
            <span className="font-medium text-foreground">
              차량분류 → 요청수량(컨)
            </span>
            {' — 차량 선택 시 비율로 요청수량이 채워집니다.'}
          </span>
          <Popover>
            <PopoverTrigger asChild>
              <Button
                ...
                aria-label="차량분류·요청수량 비율 안내"
              >
                <Info className="size-3.5" aria-hidden />
              </Button>
            </PopoverTrigger>
            <PopoverContent
              className="w-auto min-w-[10rem] p-3"
              align="start"
              side="bottom"
            >
              <ul className="list-none space-y-1.5 p-0 text-sm leading-snug tabular-nums text-foreground">
                {VEHICLE_REQUESTED_CONTAINER_QTY_HINT_ROWS.map((row) => (
                  <li key={row.vehicleLabel}>
                    {row.vehicleLabel}
                    {' → '}
                    {row.qtyLabel === '추후' ? '추후' : row.qtyLabel}
                  </li>
                ))}
              </ul>
            </PopoverContent>
          ...
          <tbody>
            {visibleRows.length === 0 && hasVisibleRowFilter ? (
              ...
              visibleRows.map((r, visibleRowIndex) => {
                ...
                const headerDisplayNumber = hasVisibleRowFilter
                  ? visibleRowIndex + 1
                  : r + 1;
                return (
              <tr
              ...
              >
                <MemoSheetRowHeader
                  row={r}
                  headerDisplayNumber={headerDisplayNumber}
                  hasVisibleRowFilter={hasVisibleRowFilter}
                  persistSaveOutcome={
                    onPersistRow ? rowPersistSaveUi[r] ?? null : null
                  }
                  className={cn(
                              ...
                              >
                                <div className="flex flex-col gap-2">
                                  <input
                                    ...
                                    onWheel={handleComboScrollWheel}
                                    aria-label={`셀 ${colLetter(c)}${r + 1} 업체 검색`}
                                  />
                                  <div className="flex items-center gap-2 text-xs text-muted-foreground">
              ...
              {sheetContextMenu.variant === 'row-header' ? (
                <>
                  <Button
                    ...
                    variant="ghost"
                    disabled={
                      !rowHeaderContextBulkTargets ||
                      rowHeaderContextBulkTargets.n === 0
                    }
                    className="h-auto w-full justify-start gap-2.5 rounded-sm px-2.5 py-2 text-sm font-normal text-foreground hover:bg-muted/80 focus-visible:bg-muted/80 focus-visible:outline-none dark:hover:bg-muted/60 disabled:opacity-50"
                    onClick={() => {
                      const t = rowHeaderContextBulkTargets;
                      if (!t || t.n === 0) return;
                      insertRowsAtFromContextMenu(t.rTop, t.n, t.rTop);
                    }}
                  >
                    <Plus
                      ...
                      strokeWidth={2}
                    />
                    {`위에 행 ${rowHeaderContextBulkTargets?.n ?? 0}개 삽입`}
                  </Button>
                  <Button
                    type="button"
                    variant="ghost"
                  ...
                  >
                    <Plus
                      ...
                      strokeWidth={2}
                    />
                    {`아래에 행 ${rowHeaderContextBulkTargets?.n ?? 0}개 삽입`}
                  </Button>
                  <Button
                    type="button"
                    variant="ghost"
                    ...
                    className="h-auto w-full justify-start gap-2.5 rounded-sm px-2.5 py-2 text-sm font-normal text-destructive hover:bg-destructive/10 hover:text-destructive focus-visible:bg-destructive/10 focus-visible:outline-none disabled:opacity-50"
                    onClick={() => deleteSelectedRowsFromContextMenu()}
                  >
                    <Trash2 className="size-4 shrink-0" aria-hidden />
                    {(rowHeaderContextBulkTargets?.n ?? 0) > 1
                      ? `행 삭제 (${rowHeaderContextBulkTargets?.n ?? 0}행)`
                      : '행 삭제'}
                  </Button>
                  <Button
                    type="button"
                  ...
                  >
                    ...
                    />
                    {(rowHeaderContextBulkTargets?.n ?? 0) > 1
                      ? `행 데이터 삭제 (${rowHeaderContextBulkTargets?.n ?? 0}행)`
                      : '행 데이터 삭제'}
                  </Button>
                </>
              ) : (
                <Button
                  type="button"
                  variant="ghost"
                  disabled={!cellContextVisibleRowCount}
                  className="h-auto w-full justify-start gap-2.5 rounded-sm px-2.5 py-2 text-sm font-normal text-destructive hover:bg-destructive/10 hover:text-destructive focus-visible:bg-destructive/10 focus-visible:outline-none disabled:opacity-50"
                  onClick={() => deleteSelectedRowsFromContextMenu()}
                >
                  <Trash2 className="size-4 shrink-0" aria-hidden />
                  {`선택한 행 삭제 (${cellContextVisibleRowCount ?? 0}행)`}
                </Button>
              )}
            </div>
          </>,
...
SalesReservationSheetGrid.displayName = 'SalesReservationSheetGrid';
