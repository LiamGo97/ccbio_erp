...
/** 신규 판매관리(/sales/management-v2) 전용 — 기존 /sales 와 분리 */
export function SalesManagementV2PageContent({
  title = '판매관리',
  description = '판매 정보를 관리합니다.',
}: {
  title?: string;
  description?: string;
} = {}) {
  const embedded = true;
  const columnSettingsInstanceId = 'sales-management-v2';
  const productLineKey = 'bl' as const;
  const useBlProductLine = true;
  const queryClient = useQueryClient();
  const columnSettings = useColumnSettings(columnSettingsInstanceId);
          ...
          <div className="rounded-md border border-border w-full min-w-0 select-text">
            <table className="w-full text-xs border-collapse table-fixed">
              <colgroup>
                ...
                <col style={{ width: '6%' }} />
                <col style={{ width: '7%' }} />
                <col style={{ width: useBlProductLine ? '14%' : '12%' }} />
                {!useBlProductLine ? <col style={{ width: '11%' }} /> : null}
                <col style={{ width: useBlProductLine ? '9%' : '8%' }} />
                <col style={{ width: '7%' }} />
                <col style={{ width: '6%' }} />
                <col style={{ width: '6%' }} />
                <col style={{ width: '4%' }} />
                    ...
                    inboundStatus === 'INBOUND_PENDING'
                      ...
                      : inboundStatus === 'INBOUND_SCHEDULED'
                        ...
                        : inboundStatus === 'INBOUND_CONFIRMED'
                          ...
                          : '';
                  const bl = useBlProductLine ? product.bl || product.bk || '-' : product.bl || '-';
  ...
  ], [router, statusMap, deliveryStatusMap, productMap, getStatusLabel, getDeliveryStatusLabel, getProductName, getSalesPriceStageName, useBlProductLine]);
  ...
  const pageBody = (
    <div className={embedded ? 'flex flex-col gap-4' : 'flex flex-col gap-4 p-4'}>
      ...
      <SalesDetailDrawer
        ...
        onConfirmDrawerOpen={handleConfirmDrawerOpen}
        productLineKey={productLineKey}
      />
    </div>
  );
