...
import { Loader2, X, Save, Plus, Trash2, FileText, Info, Search, Calculator } from 'lucide-react';
...
import { InvoiceAnalysisDrawer } from './invoice-analysis-drawer';
...
interface DocumentsProcessingFormData {
  bk?: string;
  bl?: string;
  shippingLine?: string;
  etd?: string;
...
interface DocumentsProcessingFormDrawerProps {
  open: boolean;
...
export function DocumentsProcessingFormDrawer({
  open,
  onOpenChange,
  orderId,
  onSubmit,
  onCancel,
}: DocumentsProcessingFormDrawerProps) {
  ...
  const {
    ...
    control,
    formState: { errors },
  } = useForm<DocumentsProcessingFormData>({
    defaultValues: {
      bk: '',
      bl: '',
      etd: '',
  ...
  }, [open, invoiceAnalysisDrawerOpen, trackingDrawerOpen, onOpenChange]);
  ...
  // 문서 분석 적용 여부 (적용 후 metadata 로드로 폼이 reset되는 것 방지)
  const appliedAnalysisRef = React.useRef(false);

  // 기존 데이터 로드 (의존성에 existingInvoiceFileMetadata 제외 → metadata 비동기 로드 시 폼 reset으로 컨테이너가 지워지는 버그 방지)
  React.useEffect(() => {
    if (existingOrder && open) {
      appliedAnalysisRef.current = false;
      reset({
        bk: existingOrder.bk || '',
        bl: existingOrder.bl || '',
      ...
      // 송장 파일: metadata는 별도 effect에서 비동기 로드 시 설정 (reset effect에 metadata 의존성 넣으면 로드 시 전체 폼 reset → 컨테이너 지워짐)
      setInvoiceFile(null);
    } else if (!open) {
      setInvoiceFile(null);
      appliedAnalysisRef.current = false;
  ...
  }, [open, reset, orderId, existingOrder?.id]);

  // 송장 metadata 비동기 로드 시 파일만 설정 (폼 reset 없음 → 컨테이너 유지)
  React.useEffect(() => {
    if (open && existingOrder && existingInvoiceFileMetadata && !appliedAnalysisRef.current) {
      setInvoiceFile(existingInvoiceFileMetadata);
    }
  }, [open, existingOrder, existingInvoiceFileMetadata]);
  ...
  const handleApplyAnalysisResult = React.useCallback((result: {
    ...
    if (analyzedFile === null && analysisResult === null) {
      setInvoiceFile(null);
      appliedAnalysisRef.current = true;
      return;
    }

    if (!analysisResult || !existingOrder) return;

    appliedAnalysisRef.current = true; // 이후 metadata effect가 폼을 덮어쓰지 않도록
    ...
    // 부킹 서류처리와 동일한 방식: 컨테이너 번호로 매칭하여 중량/베일만 업데이트
  ...
  const onFormSubmit = async (data: DocumentsProcessingFormData) => {
    ...
    try {
      const updateDto: any = {
        ...
        payments: (() => {
          ...
          return validPayments.length > 0 ? validPayments : null;
        })(),
        containers: (() => {
          if (!data.containers || data.containers.length === 0) {
            return [];
          }
          // 컨테이너 번호가 있는 컨테이너만 필터링하여 전송
          return data.containers
            .filter((c) => c.containerNo?.trim()) // 컨테이너 번호가 있는 것만
            .map((c) => ({
      ...
      toastSuccess('수정 완료', '서류 처리 정보가 수정되었습니다.');
      ...
      console.error('서류 처리 수정 오류:', error);
    ...
    return (
      <Drawer open={open} onOpenChange={onOpenChange}>
        <DrawerContent className="max-h-[96vh]">
          <DrawerHeader>
            <DrawerTitle>서류 처리 수정</DrawerTitle>
      ...
      >
        <DrawerHeader>
          <div className="flex items-center justify-between">
            <div className="flex-1">
              <DrawerTitle>서류 처리 수정</DrawerTitle>
              <DrawerDescription>서류 처리 정보를 수정합니다.</DrawerDescription>
            </div>
            <DrawerClose asChild>
              <Button
                type="button"
        ...
        <form onSubmit={handleSubmit(onFormSubmit)} className="flex flex-1 flex-col overflow-hidden">
          <div className="flex-1 overflow-y-auto p-4 space-y-0">
            ...
            {/* 부킹 정보 - 부킹 수정 화면과 동일 배치: 1행 순번/BK/BL/선사/도착항, 2행 ETD/ETA/현물/쿼터 */}
            ...
            <div className="space-y-3 pt-6 pb-6 border-t border-border">
              ...
              <div className="space-y-4 overflow-x-auto">
                {containerFields.map((field, index) => (
                  <div
                    key={field.id}
                    className={`grid grid-cols-[minmax(0,10rem)_minmax(0,10rem)_minmax(0,10rem)_minmax(0,8rem)_5rem_5rem_5rem_5rem_minmax(0,8rem)] gap-2 sm:gap-3 p-3 border rounded-lg items-end min-w-[max-content] ${
                      watch(`containers.${index}._fromTracking`) ? 'bg-blue-100/80 dark:bg-blue-950/40 border-blue-300 dark:border-blue-800' : ''
                    }`}
                  >
                    <div className="min-w-0 max-w-[10rem] flex flex-col gap-1.5">
                      <Label className="text-xs shrink-0 h-4 flex items-center">컨테이너 번호</Label>
                      <Input {...register(`containers.${index}.containerNo`)} className="h-9 shrink-0" />
                    </div>
                    <div className="min-w-0 max-w-[10rem] flex flex-col gap-1.5">
                      <Label className="text-xs shrink-0 h-4 flex items-center">상품</Label>
                      <Select
                    ...
                    <div className="min-w-0 max-w-[10rem] flex flex-col gap-1.5">
                      <Label className="text-xs shrink-0 h-4 flex items-center">등급(무역)</Label>
                      <Input
                    ...
                    <div className="min-w-0 max-w-[8rem] flex flex-col gap-1.5">
                      <Label className="text-xs shrink-0 h-4 flex items-center">패킹 타입</Label>
                      <Select
                    ...
                    <div className="min-w-0 max-w-[5rem] flex flex-col gap-1.5">
                      <Label className="text-xs shrink-0 h-4 flex items-center">통화단위</Label>
                      <Select
                    ...
                    <div className="min-w-0 max-w-[5rem] flex flex-col gap-1.5">
                      <Label className="text-xs shrink-0 h-4 flex items-center">단가</Label>
                      <NumberInput
                    ...
                    <div className="min-w-0 max-w-[5rem] flex flex-col gap-1.5">
                      <Label className="text-xs shrink-0 h-4 flex items-center">베일(무역)</Label>
                      <NumberInput
                    ...
                    <div className="min-w-0 max-w-[5rem] flex flex-col gap-1.5">
                      <Label className="text-xs shrink-0 h-4 flex items-center">베일(영업)</Label>
                      <NumberInput
                    ...
                    <div className="min-w-0 flex flex-col gap-1.5">
                      <Label className="text-xs shrink-0 h-4 flex items-center">중량</Label>
                      <div className="flex items-center gap-1 h-9">
                        <NumberInput
                          ...
                          step={0.001}
                          decimals={3}
                          className="h-9 flex-1 min-w-0 shrink-0"
                        />
                        <Button
                          type="button"
                          variant="ghost"
