...
interface DoProcessingFormData {
  bk?: string;
  bl?: string;
  shippingLine?: string;
  etd?: string;
...
interface DoProcessingFormDrawerProps {
  open: boolean;
...
export function DoProcessingFormDrawer({
  open,
  onOpenChange,
  orderId,
  onSubmit,
  onCancel,
}: DoProcessingFormDrawerProps) {
  ...
  const { data: existingOrder, isLoading: isLoadingOrder, refetch: refetchOrder } = useTradeOrder(orderId ?? undefined);
  ...
  const handleApplyTracking = React.useCallback(() => {
    ...
    // 컨테이너: 삭제 후 추가가 아님. 같은 컨테이너 번호면 해당 행만 업데이트(중량 등). id 유지로 판매(Sales) 연결 보존.
    // API에만 있는 건 새 행 추가.
  ...
  React.useEffect(() => {
    if (existingOrder && open) {
      // 디버깅 로그: existingOrder 확인
      console.log('[DO 처리 수정] existingOrder 로드:', {
        orderId: existingOrder.id,
        payments: existingOrder.payments,
        paymentsLength: existingOrder.payments?.length,
        paymentsArray: existingOrder.payments || [],
      });
      ...
      reset({
        ...
        payments: existingOrder.payments && existingOrder.payments.length > 0
          ? existingOrder.payments
              ...
              // DO 비용 결제 항목 제외
              .filter((p) => p.paymentType !== 'DO_COST')
              .map((p) => ({
                dueDate: p.dueDate || '',
  ...
  }, [open, reset, orderId, existingOrder?.id, existingInvoiceFileMetadata, existingDoFileMetadata]);
    ...
    if (analysisResult.containers && analysisResult.containers.length > 0) {
      ...
      analysisResult.containers.forEach((analyzedContainer: { containerNo: string | null; weight: number | null; tradeBales?: number | null; salesBales?: number | null; unitPrice?: number | null }, index: number) => {
        ...
        if (isContainerNoMismatched) {
          ...
          if (mappedContainerNo) {
            ...
            if (mappedIndex >= 0) {
              ...
              const tradeBales = analyzedContainer.tradeBales ?? undefined;
              if (tradeBales !== null && tradeBales !== undefined) {
                setValue(`containers.${mappedIndex}.tradeBales`, tradeBales);
              ...
              if (analyzedContainer.salesBales !== undefined) {
                setValue(`containers.${mappedIndex}.salesBales`, analyzedContainer.salesBales ?? undefined);
              }
        ...
        if (existingIndex >= 0) {
          ...
          const tradeBales = analyzedContainer.tradeBales ?? undefined;
          if (tradeBales !== null && tradeBales !== undefined) {
            setValue(`containers.${existingIndex}.tradeBales`, tradeBales);
          ...
          if (analyzedContainer.salesBales !== undefined) {
            setValue(`containers.${existingIndex}.salesBales`, analyzedContainer.salesBales ?? undefined);
          }
        } else {
          // 새 컨테이너 번호면: 매핑 정보 확인
          const comparison = analysisResult.containerComparisons?.[index];
          ...
          if (isNewContainer) {
            ...
            if (mappedContainerNo) {
              ...
              if (mappedIndex >= 0) {
                ...
                const tradeBales2 = analyzedContainer.tradeBales ?? undefined;
                if (tradeBales2 !== null && tradeBales2 !== undefined) {
                  setValue(`containers.${mappedIndex}.tradeBales`, tradeBales2);
                  const packingType = watch(`containers.${mappedIndex}.packingType`);
                  const exportCountryCode = existingOrder?.exportCountryCode;
                  const calculatedPacking = calculatePacking(
                    tradeBales2,
            ...
            const exportCountryCode = existingOrder?.exportCountryCode;
            const calculatedPacking = calculatePacking(
              (analyzedContainer.tradeBales ?? undefined) ?? undefined,
              defaultPackingType,
              exportCountryCode
            );
            ...
            appendContainer({
              ...
              tradeBales: (analyzedContainer.tradeBales ?? undefined) ?? undefined,
  ...
  const onFormSubmit = async (data: DoProcessingFormData) => {
    ...
    try {
      const updateDto: any = {
        ...
        payments: (() => {
          ...
          // 일반 결제 항목 (paymentType: 'REGULAR' 또는 기본값)
          if (data.payments && data.payments.length > 0) {
            const regularPayments = data.payments
              .map((payment, index) => {
                // useRatio 값 명시적으로 처리 (false도 포함)
                const useRatioValue = payment.useRatio !== undefined && payment.useRatio !== null 
                  ? payment.useRatio 
                  : true; // 기본값: true
                
                console.log(`[DO 처리] Payment ${index + 1} useRatio:`, {
                  original: payment.useRatio,
                  final: useRatioValue,
                  type: typeof payment.useRatio
                });
            ...
            allPayments.push(...regularPayments);
          }
      ...
      toastSuccess('수정 완료', 'DO 처리 정보가 수정되었습니다.');
      ...
      console.error('DO 처리 수정 오류:', error);
    ...
    return (
      <Drawer open={open} onOpenChange={onOpenChange}>
        <DrawerContent className="max-h-[96vh]">
          <DrawerHeader>
            <DrawerTitle>DO 처리 수정</DrawerTitle>
        ...
        >
          <DrawerHeader>
            <div className="flex items-center justify-between">
              <div className="flex-1">
                <DrawerTitle>DO 처리 수정</DrawerTitle>
                <DrawerDescription>DO 처리 정보를 수정합니다.</DrawerDescription>
              </div>
              <DrawerClose asChild>
                <Button
                  type="button"
          ...
          <form onSubmit={handleSubmit(onFormSubmit)} className="flex flex-1 flex-col overflow-hidden">
            ...
            >
              ...
              {/* 부킹 정보 - 서류처리/부킹 수정과 동일 배치: 1행 순번/BK/BL/선사/도착항, 2행 ETD/ETA/현물/쿼터 */}
