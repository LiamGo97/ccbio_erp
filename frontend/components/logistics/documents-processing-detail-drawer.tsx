...
import { Loader2, X, FileCheck, Edit, Trash2, FileText, Undo2, Ship } from 'lucide-react';
import {
  AlertDialog,
  AlertDialogCancel,
  AlertDialogContent,
...
import { DoProcessingDrawer } from '@/components/logistics/do-processing-drawer';
import { DocumentsProcessingFormDrawer } from '@/components/trade-order/documents-processing-form-drawer';
import { useQueryClient } from '@tanstack/react-query';

interface DocumentsProcessingDetailDrawerProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  tradeOrderId?: string | null;
  onEdit?: (order: TradeOrder) => void;
  onDelete?: (order: TradeOrder) => void;
  /** DO 처리 완료 후 호출 (상태가 DO로 변경된 order 전달, 상위에서 DO 처리 상세로 전환 시 사용) */
  onDoProcessingSuccess?: (order: TradeOrder) => void;
}

export function DocumentsProcessingDetailDrawer({
  open,
  onOpenChange,
  tradeOrderId,
  onEdit,
  onDelete,
  onDoProcessingSuccess,
}: DocumentsProcessingDetailDrawerProps) {
  ...
  const [doProcessingDrawerOpen, setDoProcessingDrawerOpen] = React.useState(false);
  ...
  const canRevertToPrevious = currentStatus === 'DOCUMENTS';
  ...
  const handleRevertToPreviousState = async () => {
    ...
    try {
      ...
      toast({
        title: '부킹으로 변경 완료',
        description: '서류 → 부킹(이전 단계)로 되돌렸습니다.',
      });
  ...
  const handleShipBackConfirm = async () => {
    ...
    const value = shipBackConfirm === 'ship_back';
    try {
      await updateOrderMutation.mutateAsync({
        id: data.id,
        data: { shipBack: value },
      ...
      toast({
        title: value ? '쉽백 처리 완료' : '쉽백 해제 완료',
        description: value ? '이 주문을 쉽백(반송)으로 표시했습니다.' : '쉽백 표시를 해제했습니다.',
      });
  ...
  React.useEffect(() => {
    ...
    const onKeyDown = (e: KeyboardEvent) => {
      ...
      if (doProcessingDrawerOpen) {
        e.preventDefault();
        setDoProcessingDrawerOpen(false);
        return;
      }
      if (editDrawerOpen) {
        e.preventDefault();
  ...
  }, [open, shipBackConfirm, doProcessingDrawerOpen, editDrawerOpen, onOpenChange]);
  ...
  return (
    <Drawer open={open} onOpenChange={onOpenChange} direction="right" dismissible={false}>
          ...
          <div className="flex items-center justify-between">
            <div className="flex-1">
              <div className="flex items-center gap-2">
                <DrawerTitle>서류 처리 상세정보</DrawerTitle>
                {isShipBack && (
        ...
        <DrawerFooter className="border-t border-border">
          <div className="flex justify-between gap-2">
            ...
            <div className="flex gap-2">
              ...
              {/* DO 처리 버튼: 무역 상태가 DOCUMENTS일 때만 표시 */}
              {data?.tradeStatus === 'DOCUMENTS' && (
                <Button
                  variant="default"
                  disabled={!data}
                  onClick={() => setDoProcessingDrawerOpen(true)}
      ...
      {/* DO 처리 Drawer */}
      <DoProcessingDrawer
        open={doProcessingDrawerOpen}
        onOpenChange={setDoProcessingDrawerOpen}
        ...
        onSuccess={async () => {
          ...
          if (updatedOrder && onDoProcessingSuccess) {
            onDoProcessingSuccess(updatedOrder);
          }
        }}
      />
