...
import { InboundConfirmedDetailDrawer } from '@/components/inbound/inbound-confirmed-detail-drawer';
...
function InboundConfirmedPageContent() {
  const queryClient = useQueryClient();
  const columnSettings = useColumnSettings('inbound-confirmed');
  ...
  const [pendingInboundEditDrawerOpen, setPendingInboundEditDrawerOpen] = React.useState(false);
  const [selectedTradeOrderForPendingInbound, setSelectedTradeOrderForPendingInbound] = React.useState<TradeOrder | null>(null);
  ...
  // 상세 drawer와 형제인 영업등급·입고 편집 drawer: Esc 시 상세 핸들러보다 먼저 닫기
  React.useEffect(() => {
    const onKeyDown = (e: KeyboardEvent) => {
      ...
      if (pendingInboundEditDrawerOpen) {
        e.preventDefault();
        e.stopImmediatePropagation();
        setPendingInboundEditDrawerOpen(false);
        return;
      }
    };
    window.addEventListener('keydown', onKeyDown, true);
    return () => window.removeEventListener('keydown', onKeyDown, true);
  }, [salesGradeEditDrawerOpen, inboundEditDrawerOpen, pendingInboundEditDrawerOpen]);

  // 제품 옵션용: 상품 필터 없이 입고확정 부킹 목록
  ...
  // 입고 확정 상태인 주문 가져오기 (salesStatus: 'INBOUND_CONFIRMED')
  ...
  const getSortValue = React.useCallback((order: TradeOrder, key: string): string | number | null => {
    ...
    if (key === 'comparisonExchangeRate') {
      const ci = order.confirmedInbound;
      return ci?.comparisonExchangeRate ?? ci?.appliedExchangeRate ?? ci?.dayExchangeRate ?? 0;
    }
    if (key === 'purchaseCost') return order.confirmedInbound?.purchaseCost ?? 0;
  ...
  /** BL 단위 목록 제외: 해당 주문의 모든 컨테이너에 excludeFromInventory 설정 후 목록 갱신 */
    ...
    async (order: TradeOrder, exclude: boolean) => {
      ...
      try {
        ...
        toast({
          ...
          description: exclude ? '이 BL의 모든 컨테이너가 재고/입고 확정 목록에서 제외됩니다.' : undefined,
        });
  ...
  const columns: ColumnDef<TradeOrder>[] = React.useMemo(() => [
    ...
    },
    {
      ...
      header: '창고',
      cell: ({ row }) => {
        const wh = row.original.confirmedInbound?.warehouse;
        const name = wh
    ...
    },
    {
      ...
      header: '이고',
      cell: ({ row }) => {
        const igodate = row.original.confirmedInbound?.igodate;
    ...
    },
    {
      ...
      header: '검역',
      cell: ({ row }) => {
        const quarantineDate = row.original.confirmedInbound?.quarantineDate;
    ...
    },
    {
      ...
      header: 'DT',
      cell: ({ row }) => {
        const dtDate = row.original.confirmedInbound?.dtDate;
    ...
    },
    {
      ...
      header: '확정 환율',
      cell: ({ row }) => {
        const inbound = row.original.confirmedInbound;
        const v = inbound?.comparisonExchangeRate ?? inbound?.appliedExchangeRate ?? inbound?.dayExchangeRate;
    ...
    },
    {
      ...
      header: '확정 원가',
      cell: ({ row }) => {
        const purchaseCost = row.original.confirmedInbound?.purchaseCost;
        if (purchaseCost == null || !Number.isFinite(Number(purchaseCost))) {
    ...
    <AppLayout user={user}>
      <div className="space-y-4 pb-4">
        <div className="flex items-center justify-between gap-2 md:items-start">
          <div>
            <h1 className="text-2xl font-bold tracking-tight">입고 확정</h1>
                    ...
                    <div className="space-y-3">
                      <div className="space-y-2">
                        <div className="flex items-center space-x-2 cursor-pointer hover:bg-muted/50 p-2 rounded">
                          <Checkbox
                            id="inbound-confirmed-product-filter-all"
                            checked={
                              availableProductCodes.length === 0 ||
                              selectedProducts.size === availableProductCodes.length
                            }
                          ...
                          />
                          <Label htmlFor="inbound-confirmed-product-filter-all" className="text-sm font-medium cursor-pointer flex-1">
                            전체
                          </Label>
                        </div>
                        {availableProductCodes.map((code) => (
                          <div key={code} className="flex items-center space-x-2 cursor-pointer hover:bg-muted/50 p-2 rounded">
                            <Checkbox
                              id={`inbound-confirmed-product-filter-${code}`}
                            ...
                            />
                            <Label htmlFor={`inbound-confirmed-product-filter-${code}`} className="text-sm font-medium cursor-pointer flex-1">
        ...
        {detailDrawerOpen && selectedTradeOrderId && (
          <InboundConfirmedDetailDrawer
            ...
            onEditPendingInboundData={(order) => {
              setSelectedTradeOrderForPendingInbound(order);
              setPendingInboundEditDrawerOpen(true);
            }}
            onSuccess={async () => {
              await queryClient.invalidateQueries({ queryKey: ['trade-orders'] });
          ...
          <InboundConfirmedEditDrawer
            ...
            onSubmit={async (formData) => {
              ...
              try {
                ...
                toast({
                  title: '오류',
                  description: '입고 확정 저장 중 오류가 발생했습니다.',
                  variant: 'destructive',
                });
              }
            }}
        ...
        {pendingInboundEditDrawerOpen && selectedTradeOrderForPendingInbound && (
          <InboundEditDrawer
            open={pendingInboundEditDrawerOpen}
            onOpenChange={(open) => {
              setPendingInboundEditDrawerOpen(open);
              if (!open) {
                setSelectedTradeOrderForPendingInbound(null);
              }
            }}
            tradeOrder={selectedTradeOrderForPendingInbound}
            labelResolvers={{
              destination: resolveDestinationLabel,
            }}
            onSubmit={async (data) => {
              if (!selectedTradeOrderForPendingInbound?.id) return;
              try {
                // 입고 예정 데이터 저장
                await api.put(`/trade/contracts/orders/${selectedTradeOrderForPendingInbound.id}/inbound`, {
                  ...data,
                  status: 'PENDING',
                });
                toast({
                  title: '성공',
                  description: '입고 예정 정보가 저장되었습니다.',
                });
                setPendingInboundEditDrawerOpen(false);
                setSelectedTradeOrderForPendingInbound(null);
                // 상세정보 drawer는 유지하고 데이터만 갱신
                ...
                console.error('입고 예정 저장 중 오류:', error);
                toast({
                  title: '오류',
                  description: '입고 예정 저장 중 오류가 발생했습니다.',
                  variant: 'destructive',
                });
              }
            }}
...
export default function InboundConfirmedPage() {
  return (
    <Suspense fallback={<div>Loading...</div>}>
      <InboundConfirmedPageContent />
    </Suspense>
  );
}
