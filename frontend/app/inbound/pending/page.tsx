...
import { TradeOrder, useTradeOrder, useTradeOrders } from '@/lib/hooks/use-trade-orders';
...
function InboundPendingPageContent() {
  const queryClient = useQueryClient();
  const columnSettings = useColumnSettings('inbound-pending');
  ...
  // 입고대기 상태인 주문 가져오기 (부킹 포함)
  // salesStatus가 null이거나 'INBOUND_PENDING'인 경우 (tradeStatus 무관)
  const { data: tradeOrders = [], isLoading, refetch } = useTradeOrders({
    bookingOnly: true,
    salesStatus: 'INBOUND_PENDING', // 입고대기 상태 (null도 포함, 부킹 포함)
  ...
  const filteredOrders = React.useMemo(() => {
    ...
    return filtered;
  }, [tradeOrders, etaStartDate, etaEndDate]);
  ...
  ], [getCodeName, salesGradeCodes, tradeStatusCodes, excludeActionOrderId, handleExcludeOrderFromList]);
  ...
  // TradeOrder를 Schedule 형태로 변환하는 함수
  const convertTradeOrderToSchedule = React.useCallback((order: TradeOrder | null): any => {
    if (!order) return null;

    // 첫 번째 컨테이너 정보 사용 (여러 개일 경우 첫 번째 것)
    ...
    return {
      ...
      commissionMonth: '',
      commissionDollar: '',
      manager: order.managerUser?.name || '',
      orderDate: order.createdAt || '',
      contractNo: order.contractNo || '',
      quota: order.quota || 'N',
      fumigation: order.fumigation || 'N',
      spot: order.spot || 'N',
      customsDuty: order.customsDuty || 'N',
      status: 'PENDING' as const,
      shippingLine: order.shippingLineName || '',
      shippingLineCode: order.shippingLineCode || null,
      shippingLineName: order.shippingLineName || null,
      shipmentSeq: order.sequence || 0,
      exporter: order.exporterName || '',
      exportCountry: order.exportCountryName || '',
      product: firstContainer?.product || order.productName || '',
      productCode: firstContainer?.product || '',
      bk: order.bk || '',
      bl: order.bl || '',
      qty: (firstContainer != null ? (firstContainer.salesBales ?? firstContainer.tradeBales) : null) ?? (order.containers?.reduce((sum, c) => sum + (c.salesBales ?? c.tradeBales ?? 0), 0) || 0),
      grade: firstContainer?.tradeGrade || null,
      packing: firstContainer?.packingType || null,
      currencyUnit: firstContainer?.currency || '',
      unitPrice: firstContainer?.unitPrice || 0,
      destination: order.destinationName || null,
      finalDestination: order.destinationName || null,
      finalDestinationArrivalDate: order.etaDate || '',
      etd: order.etdDate || '',
      eta: order.etaDate || '',
      totalAmount: order.containers?.reduce((sum, c) => sum + (c.weight || 0), 0) || 0,
      invoiceWeight: order.containers?.reduce((sum, c) => sum + (c.weight || 0), 0) || 0,
      notes: order.notes || '',
      pendingInbound: null,
                    ...
                    <div className="space-y-3">
                      <div className="space-y-2">
                        <div className="flex items-center space-x-2 cursor-pointer hover:bg-muted/50 p-2 rounded">
                          <Checkbox
                            id="inbound-pending-product-filter-all"
                            checked={
                              availableProductCodes.length === 0 ||
                              selectedProducts.size === availableProductCodes.length
                            }
                          ...
                          />
                          <Label htmlFor="inbound-pending-product-filter-all" className="text-sm font-medium cursor-pointer flex-1">
                            전체
                          </Label>
                        </div>
                        {availableProductCodes.map((code) => (
                          <div key={code} className="flex items-center space-x-2 cursor-pointer hover:bg-muted/50 p-2 rounded">
                            <Checkbox
                              id={`inbound-pending-product-filter-${code}`}
                            ...
                            />
                            <Label htmlFor={`inbound-pending-product-filter-${code}`} className="text-sm font-medium cursor-pointer flex-1">
