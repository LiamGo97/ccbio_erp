...
function DispatchRescheduledPageContent() {
    ...
    <AppLayout user={user}>
      <div className="space-y-4 pb-4">
        <div className="flex items-center justify-between gap-2 md:items-start">
          <div>
            <h1 className="text-2xl font-bold tracking-tight">일정조정</h1>
            <p className="hidden text-muted-foreground md:block">
              일정 조정 상태의 배차 정보를 조회하고 관리합니다.
            </p>
          </div>
        </div>
...
export default function DispatchRescheduledPage() {
  return (
    <Suspense fallback={<div>Loading...</div>}>
      <DispatchRescheduledPageContent />
    </Suspense>
  );
}
