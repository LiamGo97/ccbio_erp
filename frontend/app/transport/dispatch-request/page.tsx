...
function DispatchRequestPageContent() {
  ...
  const [selectedReprocessFilter, setSelectedReprocessFilter] = React.useState<string>('all');
  ...
  const filteredDispatches = React.useMemo(() => {
    ...
    // 재배차 요청 필터링
    if (selectedReprocessFilter !== 'all') {
      filtered = filtered.filter((dispatch) => {
        if (selectedReprocessFilter === 'normal') {
          // 일반 배차 (재배차 요청 이력 없음)
          return !dispatch.hasFailed && !dispatch.hasRescheduled;
        } else if (selectedReprocessFilter === 'failed') {
          // 배차실패 → 재배차 요청
          return dispatch.hasFailed === true;
        } else if (selectedReprocessFilter === 'rescheduled') {
          // 일정조정 → 재배차 요청
          return dispatch.hasRescheduled === true;
        } else if (selectedReprocessFilter === 'both') {
          // 둘 다 경험
          return dispatch.hasFailed === true && dispatch.hasRescheduled === true;
        }
        return true;
      });
    }
    ...
    return filtered;
  }, [vehicleDispatches, selectedDispatchType, selectedReprocessFilter, getDispatchType]);
  ...
  const columns: ColumnDef<VehicleDispatch>[] = React.useMemo(() => [
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        const isReprocessed = dispatch.hasFailed || dispatch.hasRescheduled;
        
        // 상태 이름 가져오기
        const failedStatusName = statusCodes?.find(c => c.value === 'FAILED')?.name || '배차 실패';
        const rescheduledStatusName = statusCodes?.find(c => c.value === 'RESCHEDULED')?.name || '일정 조정';
        
        return (
          <div className="flex items-center gap-2">
            <div className="text-sm">{dispatch.companyName || '-'}</div>
            {isReprocessed && (
  ...
  ], [requestVehicleMap, warehouseMap, statusMap, dispatchCompanyMap, statusCodes, getWarehouseName, getRequestVehicleName, getDispatchCompanyName, getStatusLabel]);
  ...
  const filterControls = (
    <div className="flex flex-wrap items-center gap-3">
      ...
      <div className="flex items-center gap-2">
        <Label htmlFor="reprocessFilter" className="whitespace-nowrap text-sm font-medium text-muted-foreground">
          재배차 요청
        </Label>
        <Select
          value={selectedReprocessFilter}
          onValueChange={(value) => {
            setSelectedReprocessFilter(value);
            setPage(1);
          }}
        >
          <SelectTrigger id="reprocessFilter" className="w-40">
            <SelectValue placeholder="전체" />
          </SelectTrigger>
          <SelectContent>
            <SelectItem value="all">전체</SelectItem>
            <SelectItem value="normal">일반 배차</SelectItem>
            <SelectItem value="failed">배차실패 → 재배차 요청</SelectItem>
            <SelectItem value="rescheduled">일정조정 → 재배차 요청</SelectItem>
            <SelectItem value="both">둘 다 경험</SelectItem>
          </SelectContent>
        </Select>
      </div>
    </div>
    ...
    <AppLayout user={user}>
      <div className="space-y-4 pb-4">
        <div className="flex items-center justify-between gap-2 md:items-start">
          <div>
            <h1 className="text-2xl font-bold tracking-tight">배차요청</h1>
            <p className="hidden text-muted-foreground md:block">
              배차 요청 상태의 배차 정보를 조회하고 관리합니다.
            </p>
          </div>
          <div className="flex items-center gap-2">
