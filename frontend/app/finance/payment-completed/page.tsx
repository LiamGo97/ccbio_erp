...
function PaymentCompletedPageContent() {
  ...
  const paymentRows = React.useMemo(() => {
    ...
    filtered.forEach((order) => {
      ...
      order.payments.forEach((payment) => {
        // 결제 결과 필터링: 완료(COMPLETED)인 것만 표시
        if (!payment.result || payment.result !== 'COMPLETED') {
          return; // 완료되지 않은 결제는 제외
        }
    ...
    return rows;
  }, [tradeOrders, selectedProduct, dueDateStartDate, dueDateEndDate]);
    ...
    <AppLayout user={user}>
      <div className="space-y-4 pb-4">
        <div className="flex items-center justify-between gap-2 md:items-start">
          <div>
            <h1 className="text-2xl font-bold tracking-tight">결제 완료</h1>
