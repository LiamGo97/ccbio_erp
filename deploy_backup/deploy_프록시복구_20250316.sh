...
# VPC all-traffic + Cloud SQL Unix socket 호환 안 됨 → 프록시 방식으로 복구
# (알리고 고정 IP는 프록시 서버 34.64.145.126에서 처리)
ALIGO_USE_PROXY=${ALIGO_USE_PROXY:-"true"}
ALIGO_PROXY_URL=${ALIGO_PROXY_URL:-"http://34.64.145.126:3000"}
...
# VPC connector 제거: all-traffic + Cloud SQL Unix socket 충돌로 배포 실패
# --clear-vpc-connector + --vpc-egress=private-ranges-only: all-traffic 상태에서 connector 제거 시 필수
# 알리고 고정 IP는 프록시 서버(ALIGO_PROXY_URL) 경유로 처리
